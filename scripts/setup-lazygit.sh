...
_fetch_remote_version() {
  ...
  if command -v curl >/dev/null 2>&1; then
    version="$(curl -s https://api.github.com/repos/jesseduffield/lazygit/releases/latest 2>/dev/null | grep -Po '"tag_name":\s*"v\K[^"]*' || true)"
  elif command -v wget >/dev/null 2>&1; then
    version="$(wget -qO- https://api.github.com/repos/jesseduffield/lazygit/releases/latest 2>/dev/null | grep -Po '"tag_name":\s*"v\K[^"]*' || true)"
  fi
