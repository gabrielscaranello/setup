# Specification: Neovim Editor & Runtime Dependencies (`scripts/setup-neovim.sh`)
...
The script SHALL ensure that `setup-nvm.sh` and `setup-go.sh` are executed across all distributions before any Neovim installation or configuration steps proceed.
...
- **Distro-specific independent dependencies**:
  - **Debian (`apt`)**: `luarocks`, `python3`, `python-venv` (`python3-venv`)
  - **Fedora (`dnf`)**: `luarocks`, `cargo`, `lua-5.1`
  - **Arch Linux (`pacman`)**: `build-tools` (`base-devel`), `rust`, `tree-sitter-cli`, `luarocks`

#### Scenario: Installing runtime packages on Debian

- **GIVEN** a Debian system
- **WHEN** `scripts/setup-neovim.sh` installs runtime dependencies
...
The script SHALL determine the Neovim binary installation mechanism based on the active package manager:
