...
The mapping file SHALL maintain pipe-separated (`|`) records with columns: `Generic Name | Debian Package | Fedora Package | Arch Linux Package`.

#### Scenario: Translating package with distro naming differences
- **GIVEN** a package has distinct naming across package managers (e.g. `neovim-runtime`, `timeshift`, `kitty`)
- **WHEN** referenced by its generic name in an installation script
- **THEN** it SHALL resolve to the exact distro-native package name specified in `packages.conf`
