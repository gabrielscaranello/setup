...
# Integration tests for scripts/system/debian/setup-kernel.sh

setup_file() {
  bash /setup/scripts/system/debian/setup-kernel.sh
}

@test "setup-kernel.sh executes successfully" {
  run bash /setup/scripts/system/debian/setup-kernel.sh
  [ "$status" -eq 0 ]
}
